"""
维修过程记录（case state）服务

从 app.py 下沉的全部维修过程记录业务逻辑：
- case_state 的默认结构、旧数据兼容归一化
- 故障上下文注入（问答约束）、拒答识别
- LLM 抽取：关键词 / 故障部件 / 待确认草案
- 维修记录（maintenance_record）与复盘（postmortem）的生成
- 草案应用（apply-draft）

对话数据统一经由 conversation_service（SQLite）读写。
"""
import json
import logging
import re
...
def default_case_state() -> Dict[str, Any]:
    return {
        ...
        "confirmed_measures": [],  # 已执行 [{id, text, created_at, executed_at, status='executed'}]
        "keywords": [],  # [str]
...
def ensure_conversation_case_state(conversation_id: str) -> None:
    """确保对话存在且 case_state 结构完整（含旧数据兼容归一化）"""
...
def _generate_freeform_text(prompt: str) -> str:
    """生成不依赖检索文档的自由文本（用于复盘等），复用全局 RAG 系统的 LLM 配置"""
    try:
        rag_system = get_rag_system()
    except Exception:
        return ""
    if not getattr(rag_system, "generation_module", None):
        return ""
...
def generate_case_draft_from_conversation(conversation_id: str) -> Dict[str, Any]:
    """使用 LLM 从当前对话生成“待用户确认”的故障草案（不自动生效）。"""
    ensure_conversation_case_state(conversation_id)
    conv = _conv(conversation_id)
    state = conv["case_state"]
    messages = conv.get("messages", []) or []
...
def build_maintenance_record(conversation_id: str) -> Dict[str, Any]:
    """生成维修记录 markdown 并写入 case_state，返回最新 case_state"""
    ensure_conversation_case_state(conversation_id)
    conv = _conv(conversation_id)
    state = conv["case_state"]
...
def build_postmortem(conversation_id: str) -> Dict[str, Any]:
    """生成复盘 markdown 并写入 case_state，返回最新 case_state"""
    ensure_conversation_case_state(conversation_id)
    conv = _conv(conversation_id)
    state = conv["case_state"]
...
def apply_draft(conversation_id: str,
                fault_context: Optional[Dict[str, Any]] = None,
                measures: Optional[List[str]] = None,
                todos: Optional[List[str]] = None,
                components: Optional[List[str]] = None,
                keywords: Optional[List[str]] = None,
                confirm_fault_context: bool = False) -> Dict[str, Any]:
    """接受（可部分拒绝后的）草案内容，合并到正式 case_state，返回最新 case_state"""
    ensure_conversation_case_state(conversation_id)
    conv = _conv(conversation_id)
    state = conv["case_state"]
    ...
    fc_req = fault_context if isinstance(fault_context, dict) else {}
    ...
    accepted_measures = _norm_list(
        measures if measures is not None else draft.get("suggested_measures") or []
    )
    ...
    accepted_todos = _norm_list(
        todos if todos is not None else draft.get("suggested_todos") or []
    )
    ...
    accepted_keywords = _norm_list(
        keywords if keywords is not None else draft.get("keywords") or [],
        to_fault=True
    )
    ...
    accepted_components = _norm_list(
        components if components is not None else (draft.get("suggested_components") or [])
    )
